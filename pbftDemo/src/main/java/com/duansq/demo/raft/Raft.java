...
public class Raft implements IRaft {
    ...
    private volatile int currentTerm; // 任期

    public Raft(int node, int size) {
        this.index = node;
        this.size = size;
        this.maxf = size / 2;
    }
...
//				close();
    ...
    private void onCopyRes(RaftMsg msg) {
        commited.put(msg.getId(), msg.getData());
    ...
    private void onCopyReq(RaftMsg msg) {
        ...
        for (int i = msg.getId(); i < Integer.parseInt(msg.getData()); i++) {
            ...
            String data = commited.get(msg.getId());
            rsp.setData(data);
            rsp.setId(i);
    ...
    private void commit() {
        // 请求被允许，可放心执行
        commited.put(this.commitIndex, curData);
