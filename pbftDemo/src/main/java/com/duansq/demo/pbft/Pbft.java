...
public class Pbft {
    ...
    //节点出块的序列号
    private volatile AtomicInteger blockNumber = new AtomicInteger(0);
    //统计通信量
    public volatile static AtomicInteger transCount = new AtomicInteger(0);
    ...
    private void onReq(PbftMsg msg) {
        ...
        if (msg.getVnum() == index) {
            if (applyMsg.containsKey(msg.getDataKey())) {
                return; // 已经受理过
            }
...
//			sed.setVnum(view);
    ...
    private void onReply(PbftMsg msg) {
        if (curMsg == null || !curMsg.getData().equals(msg.getData())) {
            return;
        }
    ...
    private void doSomething(PbftMsg msg) {
        ...
        //transCount.getAndIncrement();
        int getBlockNumber = blockNumber.incrementAndGet();
        logger.info("第" + index + "节点出块" + "块的序列号为" + getBlockNumber);
        //transCount.getAndIncrement();
